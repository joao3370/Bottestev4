...
const { destrava } = require('./src/destrava')         /* ESTA VERSÃƒO DESSE SOFTWARE Ã‰ PAGA, NÃƒO DIVULGUE DE FORMA GRATUITA. */
...
const { y2mateA, y2mateV } = require('.database/json')
...
/*const { xp } = require('./database/menu/xp') (resolver bug xp) 
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:criador\n' // nome do dono (Joao) 
            + 'ORG:bot;\n' // Bot
            + 'TEL;type=CELL;type=VOICE;waid=+554891635455 :+55 48991635455 \n' //NÃºmero do zape
            + 'END:VCARD'
prefix = '/'
emoji = 'ðŸ¦†' 
botname = 'bot' 
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Ola @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\n\nEspero que goste, leia as regras. digite /help para conhecer mais sobre meus comandos`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["554891635455@s.whatsapp.net","17404627820@s.whatsapp.net"] // nÃºmero de dono 
			const mod = [ownerNumber,"554891635455@s.whatsapp.net","17404627820@s.whatsapp.net"]//nÃºmero de mod/dono 
			const adminbotnumber = ["554891635455@s.whatsapp.net","17404627820@s.whatsapp.net"]//nÃºmero de adm
			const frendsowner = ["554891635455@s.whatsapp.net","17404627820@s.whatsapp.net"]//nÃºmero de "amigos" (arrumar definiÃ§Ã£o) 
			const premium = ["554891635455@s.whatsapp.net","17404627820@s.whatsapp.net"] // Premium
        ...
        let id_vote = sender ? sender : '5548991635455@s.whatsapp.net'
        if(fil.includes(id_vote)) {
        return mentions('@'+sender.split('@')[0]+' Voce votou', fil, true)
        } else {
        vote.push({
            participant: id_vote,
            voting: ''
        ...
        let _p = []
        let _vote = '*Vote* '+ '@'+ _votes[0].votes.split('@')[0] + `\n\n*Motivo*: ${_votes[0].reason}\n*Numero de votos* : ${vote.length} Vote\n*Duracao* : ${_votes[0].duracao} Minuto\n\n` 
		...
		if (isGroupAdmins) return reply('cara, nao fale essas coisas, isso é errado')
	...
	if (messagesC.includes("jesuschorou")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/jesuschorou.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
	if (messagesC.includes("vidaloka1")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/vidaloka1.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
	if (messagesC.includes("vidaloka2")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/vidaloka2.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
			...
			switch(command) {
				...
				case 'menu':
                    ...
                    uptime=process.uptime 
                       client.sendMessage(from, help(prefix, emoji), text, {quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "*Joao BOT*", 'jpegThumbnail': fs.readFileSync('./assets/botlogo.webp')}}}})
                    break
                    case 'delvote':
            if(!Kyz.key.remoteJid) return
            if(isVote) return reply('Nao ha votacao')
            delVote(from)
            reply('Eliminado com sucesso a sessão de votação neste grupo')
            break
    case 'voting':
            ////if(!isGroupAdmins) return 
            if(!isGroup) return reply(mess.only.group)
            if (isVote) return reply('Sessao de votos em andamento')
            if(!q) return reply('*Voting*\n\n'+ prefix+ 'voting @tag target | reason  | 1 (1 = 1 Minuto)\nExemplo: /voting @tag | motivo | hora da votacao')
            ...
            if(!Number(split[2])) return reply('insira um numero 3\nExemplo: 1-9999\n1 = 1 Minuto')
            await mentions('Vote ' +'@'+ id.split('@')[0]+' Inicio' +'\n\n' + `vote = \ndevote = \n\nMotivo: ${split[1]}`,[id],true)
		...
		case 'iri':
			...
			break
                ...
                case 'quotemaker':
					...
					break
				case 'galaxtext':
					if (args.length < 1) return reply('o que vocÃª quer')
					teks = body.slice(12)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'water':
                 ...
                 case 'nomegp':
		             ...
		             break
				case 'loli2':
					if (!isAnime) return reply('âŒ *Deve ativar o modo Anime* âŒ')
...
//@darkYT					
    ...
    break
				...
				case 'semoji':
					if (args.length < 1) return reply('onde estÃ¡ o emojis?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'premiumlist':
					...
					teks += `â”‚+ Total : ${premium.length}\nâ•°â”€â”€â”€â”€â”€â”€*ã€Œ *BOT* ã€*â”€â”€â”€â”€`
                ...
                case 'travar':
                     const pesan = body.slice(5)
                      ...
                      if (pesan.length > 5000) return client.sendMessage(from, 'ate 5k de caracteres', msgType.text, {quoted: mek})
                       ...
                       const teks1 = `*[TRAVAðŸŠðŸš©]*\nTRAVADO POR : @${nomor.split("@s.whatsapp.net")[0]}\nMensagem : ${pesan}`
                       const teks2 = `*[TRAVAðŸŠðŸš©]*\nTRAVADO POR : @${nomor.split("@s.whatsapp.net")[0]}\nMensagem : ${pesan}`
                       const teks3 = `*[TRAVAðŸŠðŸš©]*\nTRAVADO POR : @${nomor.split("@s.whatsapp.net")[0]}\nMensagem : ${pesan}`
                       const teks4 = `*[TRAVAðŸŠðŸš©]*\nTRAVADO POR : @${nomor.split("@s.whatsapp.net")[0]}\nMensagem : ${pesan}`
                      var options = {
                         text: teks1,
                         text: teks2,
                         text: teks3,
...
break
		   ...
		   await limitAdd(sender)
			...
			case 'instaimg':
			    ...
			    case 'nsfwblowjob':
				    try {
						if (!isNsfw) return reply('âŒ *FALSO* âŒ')
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`â•­â”€â”€â”€â”€*ã€Œ *TOTAL DE USUÃRIOS BOT ðŸ‘‘* ã€\n\`\`\``
					no = 0
					for (let hehehe of user) {
						no += 1
						teks += `\`\`\`[${no.toString()}]\`\`\` @${hehehe.split('@')[0]}\n`
					}
					teks += `â”‚+ Total de usuÃ¡rios : ${user.length}\nâ•°â”€â”€â”€â”€â”€â”€*ã€Œ *JOAO* ã€*â”€â”€â”€â”€`
...
// recursos anime, random @Ola
              ...
              case 'cerpen':
               ...
               case 'puisiimg':
			    ...
			    case 'randomhentai':
				    try {
						if (!isNsfw) return reply('âŒ *FALSE* âŒ')
			...
			break
               ...
               if (anu.error) return reply(anu.error)
			    ...
			    case 'criador':
                    ...
                    client.sendMessage(from, 'Ctt do criador',MessageType.text, { quoted: mek} )
                    break
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								fs.unlinkSync(media)
								tipe = media.endsWith('.mp4') ? 'video' : 'gif'
								reply(`âŒ Falhou, no momento da conversÃ£o ${tipe} para o adesivo`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('VocÃª nÃ£o Ã© um Membro Premium, entre em contato com o dono ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*â•­â”€â”€â”€â”€*ã€Œ *ADMINBOT joao âœ¨* ã€\n*â”‚+ wa.me/5548991635455*â•°â”€â”€â”€â”€â”€â”€*ã€Œ *joao* ã€*â”€â”€â”€â”€*\n\n*_SE QUER SER ADMIN DO BOT joao_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				break
				case 'Pedro':                 
				 data = fs.readFileSync('./src/Pedro.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
                sendImage(hasil, mek, '*Pedro :V*')
				break
				case 'belle1':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ï¸*canal do joao:*\n\n https://youtube.com/'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.ibb.co/x5BrL2V/6e98508e0ae80ec8480507601b5978f9402230d1-00.gif`)
				...
				case 'loli':
					loli.getSFWLoli(async (err, res) => {
						if (err) return reply('âŒ *ERROR* âŒ')
				...
				case 'nsfwloli':
					...
					loli.getNSFWLoli(async (err, res) => {
						if (err) return reply('âŒ *ERRO* âŒ')
						buffer = await getBuffer(res.url)
...
break
				...
				case 'wait':
					...
					break
				default:
				if (budy === `${prefix}${command}`) {
reply(`Comando ${command} NÃ£o Encontrado, Pesquisando Alguma Coisa Sobre Isso!!`)
causs.updatePresence(from, Presence.composing) 
data = await fetchJson(`https://fdciabdul.tech/api/pinterest?keyword=${command}`, {method: 'get'})
n = JSON.parse(JSON.stringify(data));
nimek =  n[Math.floor(Math.random() * n.length)];
pok = await getBuffer(nimek)
causs.sendMessage(from, pok, image, { quoted: mek, caption: `Pesquisa Feita Sobre ${command}`})		
}
				}
